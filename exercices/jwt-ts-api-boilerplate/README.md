...
## Description
Cette documentation décrit les différentes opérations mises à disposition par l'API pour gérer les films et les commentaires, ainsi que les endpoints d'authentification. Les opérations protégées par JWT nécessitent qu'un utilisateur soit authentifié.
...
| **`/films/:id`**   | GET          | Non      | **READ ONE** : Lire une ressource spécifique de la collection.                                                     |
| **`/films`**       | POST         | JWT      | **CREATE ONE** : Créer un film (body : `{title: string; director: string; duration: number; budget?: number; description?: string; imageUrl?: string}`). |
| **`/films/:id`**   | PUT          | JWT      | **UPDATE ONE** : Modifier un film existant (body similaire à POST).                                                |
| **`/films/:id`**   | DELETE       | JWT      | **DELETE ONE** : Supprimer un film existant.                                                                       |
| **`/comments`**    | GET          | Non      | **READ ALL FILTERED** : Lire tous les commentaires, avec option de filtre par film.                                |
| **`/comments`**    | POST         | JWT      | **CREATE ONE** : Ajouter un commentaire à un film, en associant l'utilisateur authentifié (body : `{content: string; filmId: string}`). |
| **`/comments/:id`**| DELETE       | JWT      | **DELETE ONE** : Supprimer un commentaire sur base de l'utilisateur authentifié.                                   |
| **`/auth/login`**  | POST         | Non      | **LOGIN** : Authentifier un utilisateur (body : `{email: string; password: string}`).                              |
| **`/auth/register`** | POST       | Non      | **REGISTER** : Créer un nouvel utilisateur (body : `{name: string; email: string; password: string}`).             |

---

## Exemples de corps de requête

### Créer un film (POST `/films`)
```json
{
  "title": "Inception",
  ...
  "duration": 148,
  "budget": 160000000,
  "description": "Un voleur qui s'infiltre dans les rêves des autres pour voler des secrets."
}
```
...
```json
{
  "content": "Super film, je recommande !",
  "filmId": "123456789"
}
```
...
## Notes
- Toutes les routes **protégées par JWT** nécessitent un token valide dans l'en-tête `Authorization` :
  ```http
  Authorization: Bearer <votre_token>
  ```
- Les routes **de lecture** (GET) pour les films et commentaires sont publiques.

---

## Authentification
### Génération du token JWT
Lors de la connexion (à l'aide de `/auth/login`), un token JWT est retourné. Ce token doit être inclus dans l'en-tête pour accéder aux routes protégées.
